...
train_data_conf_matrix = classification_report(y_train, model.predict(x_train), target_names=['not_off', 'off'])
print(train_data_conf_matrix)
print('test data confusion matrix')
test_data_conf_matrix = classification_report(y_test, model.predict(x_test), target_names=['not_off', 'off'])
print(test_data_conf_matrix)
print('eval data confusion matrix')
eval_data_conf_matrix = classification_report(y_eval, model.predict(x_eval), target_names=['not_off', 'off'])
...
if user_input.lower() == 'y':
    ...
    with open('./results.txt', 'a') as results_file:
        results_file.write(f'\n{model_filename} for offensive task results:\n')
