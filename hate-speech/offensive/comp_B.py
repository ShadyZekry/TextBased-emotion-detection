...
hs_lr = pickle.load(open('./lr_hs_model.sav', 'rb'))
lr_predictions = hs_lr.predict(test_features)

with open('./CHILLAX_subtaskB_1.txt', 'w') as submit_file:
    for prediction in lr_predictions:
        if prediction == 0:
            submit_file.write('NOT_HS\n')
        elif prediction == 1:
            submit_file.write('HS\n')

hs_rf = pickle.load(open('./rf_hs_model.sav', 'rb'))
rf_predictions = hs_rf.predict(test_features)

with open('./CHILLAX_subtaskB_2.txt', 'w') as submit_file2:
    for prediction in rf_predictions:
        if prediction == 0:
            submit_file2.write("NOT_HS\n")
        elif prediction == 1:
            submit_file2.write("HS\n")
