...
train_df = pd.read_csv('./result/augmented_offensive_data.csv', usecols=['label']).dropna()
val_df = pd.read_csv('./dataset/OSACT2022-sharedTask-dev.csv', usecols=['off_label']).dropna()
...
off_count = train_df.loc[train_df['label'] == 1].shape[0]
print(f'Offensive tweets in train: {off_count}')

non_off_count = train_df.loc[train_df['label'] == 0].shape[0]
