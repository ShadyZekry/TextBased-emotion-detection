...
y_eval = to_categorical(y_eval, num_classes=6)
...
print('train data:')
train_data_conf_matrix = classification_report(np.argmax(y_train, axis=-1), np.argmax(model.predict(x_train), axis=-1), target_names=['not_hs', 'hs1', 'hs2', 'hs3', 'hs5', 'hs6'])
print(train_data_conf_matrix)

print('test data:')
test_data_conf_matrix = classification_report(np.argmax(y_test, axis=-1), np.argmax(model.predict(x_test), axis=-1), target_names=['not_hs', 'hs1', 'hs2', 'hs3', 'hs5', 'hs6'])
print(test_data_conf_matrix)

print('eval data:')
eval_data_conf_matrix = classification_report(np.argmax(y_eval, axis=-1), np.argmax(model.predict(x_eval), axis=-1), target_names=['not_hs', 'hs1', 'hs2', 'hs3', 'hs5', 'hs6'])
print(eval_data_conf_matrix)

user_input = input('Save model?..(y/n)')
if user_input.lower() == 'y':
    model_json = model.to_json()
    with open(f'./{model.name}_multiclass.json', 'w') as json_file:
        json_file.write(model_json)
    model.save_weights(f'./{model.name}_multiclass.h5')
    with open('./results.txt', 'a') as results_file:
        results_file.write(f'\n{model.name} for multiclass task results:\n')
