...
train_df = pd.read_csv('./result/augmented_hatespeech_data.csv', usecols=['tweet']).dropna()
val_df = pd.read_csv('./result/OSACT2022-sharedTask-dev.csv', usecols=['tweet']).dropna()
...
train_features = np.empty(shape=(x_train_texts.shape[0], 768), dtype=np.float64)
...
val_features = np.empty(shape=(x_val_texts.shape[0], 768), dtype=np.float64)
...
save_train_features(train_features, 'hs_')
save_val_features(val_features, 'hs_')
